...
# This script expects a running 11n10pr network and will create a continuous stream of 200 send & open blocks.
# It will wait for the previous confirmation (or timeout) before publishing the next block

CONFIRMATION_TIMEOUT_SECONDS = 20
THRESHOLD = 803000
LEGIT_BLOCKS_PATH = "_resources/blocks/11n10pr_200legit.json"
RPC_ENDPOINTS = [
    ...
    "http://127.0.0.1:45106",
    "http://127.0.0.1:45107",
    "http://127.0.0.1:45108",
    "http://127.0.0.1:45109",
...
async def confirm_single_block(rpc, hash, timeout, start_times):
    ...
    return {
        'seen': f"{seen_time:.2f}" if seen_time else "N/A",
        'confirmed': f"{confirmed_time:.2f}" if confirmed_time else "N/A",
